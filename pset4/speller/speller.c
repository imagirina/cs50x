...
#undef calculate
#undef getrusage //with function getrusage we’ll be "benchmarking" (i.e., timing the execution of) your implementations of check, load, size, and unload
...
int main(int argc, char *argv[])
{
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
        ...
        else if (index > 0)
        {
            ...
            getrusage(RUSAGE_SELF, &before);
            bool misspelled = !check(word); // notice how we go about passing check, word by word, the contents of some file to be spell-checked
